...
(function () {
  ...
  const inpNodeSize = document.getElementById('inp-node-size');
  ...
  function renderNodes() {
    ...
    state.nodes.forEach(node => {
      const sizeMode = node.sizeMode || 'auto';
      const el = document.createElement('div');
      el.className = `flow-node ${node.theme || 'purple'} ${node.id === state.selectedNodeId ? 'selected' : ''} status-${node.status || 'idle'} size-${sizeMode}`;
      el.id = node.id;
      el.style.left = `${node.x}px`;
      el.style.top = `${node.y}px`;

      if (sizeMode === 'custom' && node.customWidth) {
      ...
      el.innerHTML = `
        ...
        <div class="node-resize-handle" title="Tarik untuk memperlebar / memperkecil node"></div>
      ...
      // Setup Drag Resizing for Node
      const resizeHandle = el.querySelector('.node-resize-handle');
      setupNodeResizing(node, resizeHandle, el);
  ...
  function setupNodeResizing(node, handleEl, nodeEl) {
    if (!handleEl) return;
    handleEl.addEventListener('mousedown', (e) => {
      ...
      function onMouseMove(moveEvent) {
        ...
        const newW = Math.max(160, Math.round(startWidth + dx));
        const newH = Math.max(80, Math.round(startHeight + dy));
        ...
        nodeEl.className = `flow-node ${node.theme || 'purple'} ${node.id === state.selectedNodeId ? 'selected' : ''} status-${node.status || 'idle'} size-custom`;
  ...
  function updateInspectorForm() {
    ...
    inpNodeSize.value = node.sizeMode || 'auto';
  ...
  function setupInspectorEvents() {
    ...
    inpNodeSize.addEventListener('change', () => {
      const node = state.nodes.find(n => n.id === state.selectedNodeId);
      if (!node) return;

      node.sizeMode = inpNodeSize.value;
      if (node.sizeMode !== 'custom') {
  ...
  function setupSidebarResizer() {
    ...
    resizer.title = 'Tarik untuk mengubah lebar Sidebar';
    ...
    resizer.addEventListener('mousedown', (e) => {
      ...
      function onMouseMove(moveEvent) {
        ...
        const newWidth = Math.min(450, Math.max(200, moveEvent.clientX));
